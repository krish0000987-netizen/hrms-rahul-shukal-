"""
Rahul HRMS management command to create a new user and associated employee.
"""
...
class Command(BaseCommand):
    ...
    help = "Creates a new Rahul HRMS administrator/employee user"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            bot = HorillaUser.objects.filter(username="Rahul HRMS Bot").first()
            if bot is None:
                HorillaUser.objects.create_user(
                    username="Rahul HRMS Bot",
                    password=str(uuid.uuid4()),
                )

            self.stdout.write(
                self.style.SUCCESS(f'Rahul HRMS Employee "{employee}" created successfully')
            )
